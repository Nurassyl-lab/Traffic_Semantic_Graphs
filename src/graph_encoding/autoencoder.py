...
class HeteroGraphAutoencoder(nn.Module):
    def __init__(self, metadata, hidden_dim=64, embed_dim=32, quantizer_spec=None, feat_emb_dim=16, 
                 ...
                 num_encoder_layers=1, num_decoder_layers=1, activation='relu', dropout_rate=0.1, side_info_dim=0):
        super().__init__()
        ...
        if self.side_info_dim > 0:
            self.side_info_proj = Linear(side_info_dim, hidden_dim)
        ...
        for nt, spec in quantizer_spec.items():
            ...
            for i in range(num_decoder_layers -1):
                decoder_layers.append(Linear(embed_dim, embed_dim))
                decoder_layers.append(self.activation)
            decoder_layers.append(Linear(embed_dim, Fdim * bins))
            self.feature_heads[nt] = nn.Sequential(*decoder_layers)
    ...
    def encode(self, x_dict, edge_index_dict, batch_dict=None, side_info=None):
        ...
        for nt, pre in self.pre_lin.items():
            if nt in x_dict:
                x0[nt] = self.activation(pre(x_dict[nt]))
            else:
                continue
        ...
        for conv in self.encoder_layers:
            ...
            h = {key: self.dropout(self.activation(val)) for key, val in h.items()}

        # if no messages for a node type, use pre-projected features x0
        h_dict = {}
        for nt in x0.keys():
            h_dict[nt] = h.get(nt, x0[nt])

        # Final projection to latent
        z_dict = {nt: self.activation(self.lin_proj[nt](h)) for nt, h in h_dict.items()}
        return z_dict
...
def batched_graph_embeddings(z_dict, batch_data, metadata, pooling="mean", embed_dim_per_type=32):
    ...
    for nt in metadata[0]:
        if nt in z_dict and "batch" in batch_data[nt]:
            ...
            if batch_vec.numel() == 0: # Add this check
                B_nt = 0
            else:
